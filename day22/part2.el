...
;;
;; Before beginning our calculation, we partition the grid into 8
;; segments (one for each axis-aligned plane) at the median
;; coordinates. And then, well, we repeatedly do that until we have
;; small enough sets. Partitioning once divides the runtime by b^8
;; (where b is our branching factor) and multiplies it by 8, so it's
;; definitely worth it.
