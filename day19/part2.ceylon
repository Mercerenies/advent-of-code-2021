...
class Point(x, y, z) {
    ...
    shared Integer distance(Point that) =>
        (this.x - that.x).magnitude + (this.y - that.y).magnitude + (this.z - that.z).magnitude;
...
shared void part2() {
    ...
    variable Integer largestDistance = 0;
    for ([point1, _s1] in assignedScanners) {
        for ([point2, _s2] in assignedScanners) {
            largestDistance = max([largestDistance, point1.distance(point2)]);
        }
    }
